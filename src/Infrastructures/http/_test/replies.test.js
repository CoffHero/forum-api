...
describe('/threads/{threadId}/comments/{commentId}/replies endpoint', () => {
  ...
  describe('when DELETE /threads/{threadId}/comments/{commentId}/replies/{replyId}', () => {
    ...
    it('should response 403 status code when user is not the owner of the reply', async () => {
      ...
      const notOwner = 'user-456'
      await UsersTableTestHelper.addUser({ id: notOwner, username: 'user456' })
      const replyId = 'reply-456'
      await RepliesTableTestHelper.addReply({ id: replyId, commentId, owner: notOwner })
    ...
    it('should response 404 status code when thread, comment or reply is not present or is not valid', async () => {
      // Arrange
      const invalidThreadId = 'thread-777'
      const invalidCommentId = 'comment-777'
      const invalidReplyId = 'reply-777'
